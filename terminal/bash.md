...
locate -e *.conf                           # check files exist before showing result
```
...
find /path -name "file?.txt"               # Find files with 'file' in the file name and '1' num\char after
...
find / -type f -size +100k                 # Find based on file size
sudo find / -type f -size +100k -size -5M -exec cp {} <path> \; #find and copy to destination
sudo find / -type f -size +100k -size -5M -ok cp {} <path> \;   #find and copy to destination (ask for permission)
```
